...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        attn = torch.zeros(self.depth, B, self.num_heads, x.shape[1], x.shape[1], device=x.device)
        for i, blk in enumerate(self.blocks):
            x, attn[i] = blk(x, return_attention=True)
        x = self.norm(x)
    ...
    def forward(self, x):
        ...
        output_embed = x[:,1:,:].clone().detach() # Bx197x192
        x = self.head(x[:,0])
        return x, attn, output_embed
